...
interface DemoStoreData {
  ...
  totalUtang: number;
  /** Top products by demo_products.sold_count -- see migration 20260815141000. */
  bestSellers: DemoProduct[];
}

/**
 * Reads the shared, seeded, read-only public.demo_* tables -- same tables
 * the web app's Demo Store reads (apps/tindahan-pos/src/lib/demoData/demoData.ts).
 * Never touches a real store's data: these tables have no store_id column
 * and no client write grant at all.
 */
export function useDemoStoreData(): DemoStoreData {
  ...
  return {
    ...
    bestSellers: [...products].sort((a, b) => b.soldCount - a.soldCount).slice(0, 4),
  };
}

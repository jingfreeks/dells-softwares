 ...
 * past their credit limit. This is server-enforced by checkout_sale() (see
 * 0022_owner_pin_override.sql in the web app's migrations) — callers use
 * this for instant client-side UX (showing a warning before even
 * attempting checkout), but the real authorization decision is made
 * server-side against the customer's authoritative, row-locked balance.
 * A customer with no limit set never triggers this.
 */
...
/**
 * Days since this customer's earliest recorded "credit" (utang) sale — a
 * stand-in for "how old is the oldest unpaid debt". The backend has no
 * ledger linking a payment to the specific sale it settles (only a running
 * balance), so this is an approximation from real sales data rather than
 * an exact unpaid-since date. Ported from
 * apps/tindahan-pos/src/lib/customers/customers.ts. Returns null when the
 * customer has no balance or no credit sales.
 */
...
/** Debt older than `thresholdDays` (default 30) counts as "overdue". */
 ...
 * `thresholdDays` (default 30) as the "overdue" cut and its midpoint as the
 * boundary between the first two buckets. The `bucket0to14`/`bucket15to30`
 * field names are historical (from the web app's original hardcoded
 * 14/30-day version) but now hold whatever range the threshold produces.
 */

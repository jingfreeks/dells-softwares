...
interface AuthContextValue {
  ...
  /** Creates a brand-new store and its admin account — this app is sold
   * standalone, so it does not require a prior tindahan-pos signup. */
  register: (input: {
    storeName: string;
    ownerName: string;
    email: string;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <AuthContext.Provider
      value={{ user, store, loading, login, register, logout, requestPasswordReset }}
    >
      {children}
    </AuthContext.Provider>
  );

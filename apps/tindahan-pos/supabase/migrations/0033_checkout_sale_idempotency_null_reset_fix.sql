-- 0033_checkout_sale_idempotency_null_reset_fix.sql
--
-- Second bug found during manual verification (companion to 0032's fix):
-- the idempotent-replay lookup did
--   select sales.id, sales.total into v_sale_id, v_total from sales where ...
-- On a genuine cache miss (no prior sale with this client_request_id —
-- the normal case for every first-time checkout), that SELECT returns zero
-- rows. Per plpgsql's documented behavior, a no-row SELECT INTO doesn't
-- leave its targets untouched — it resets ALL of them to NULL. That
-- silently clobbered v_total's `:= 0` initializer before the item-pricing
-- loop ever ran, so `v_total := v_total + v_line_total` became
-- `null + 1.67 = null` for the rest of the function, and the final insert
-- failed with "null value in column total ... violates not-null
-- constraint" on every single checkout, not just replays.
--
-- Fix: look up into dedicated scratch variables that only exist for this
-- check, and only copy them into v_sale_id/v_total inside the `if found`
-- branch — a miss now leaves v_sale_id/v_total exactly as declared.
...
begin
  ...
  if p_client_request_id is not null then
    select sales.id, sales.total into v_existing_sale_id, v_existing_total
      from sales
      where sales.client_request_id = p_client_request_id and sales.store_id = v_store_id;
    if found then
      return query select v_existing_sale_id, v_existing_total;
      return;
    end if;
  end if;

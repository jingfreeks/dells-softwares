-- 0041_sale_device_traceability.sql
--
-- BIR Compliance §49 "POS Device Registration": every transaction should be
-- traceable to Business -> Branch -> POS Device -> Cashier -> Transaction ->
-- Invoice. `sales.cashier_id` already records who rang up a sale, but
-- nothing records which physical/paired device it happened on.
--
-- This app already has a device-pairing system (devices table,
-- 0026_device_pairing.sql) — a paired tablet authenticates as its own
-- auth.users/devices row, distinct from a staff row, and auth_store_id()
-- already resolves a store for either kind of caller. The gap is narrow:
-- checkout_sale() never captured the calling device's identity onto the
-- sales row it creates.
--
-- Note the POS checkout screen is always gated behind a cashier login
-- (CashierLoginScreen), so a cashier_token is always present by the time a
-- real checkout fires — there's no "raw device sale with no cashier
-- attached" case to special-case here. This is purely additive: capture
-- the calling device's id (independent of whichever staff member's
-- cashier token is attached) into a new sales.device_id column.

alter table sales
  add column device_id uuid references devices (id);

-- create or replace, not drop+create: signature/returns-table shape are
-- unchanged from 0040_vat_computation.sql — device_id is only ever written
-- into `sales`, never part of the RPC's own output.
...
begin
  ...
  -- Traceability (BIR §49): if the RPC caller is a paired device (not a
  -- plain staff sign-in), record which one. Mirrors auth_store_id()'s own
  -- devices branch exactly. auth.uid() is stable for the whole call, so
  -- this doesn't need to run before p_cashier_token reassigns v_cashier_id
  -- below — they're independent identities (device vs. attributed cashier).

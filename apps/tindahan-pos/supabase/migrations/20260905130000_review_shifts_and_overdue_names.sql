-- Review learns about shifts, and who to chase
--
-- The last two figures the dashboard's sections need that the aggregate did
-- not yet produce. Backend only; the PR that draws the sections follows.
--
-- CASHIER SHIFTS
--
-- The design's fourth attention item reads "Cashier shifts are balanced -- no
-- action needed". Balanced against WHAT was the open half of design gap 3.2:
-- the threshold lived in the client's localStorage until 20260905100000 made
-- it stores.drawer_variance_threshold. This is the half that reads it.
--
-- "Closed and counted" is `variance is not null`. A session still open, or
-- abandoned without a closing count, has nothing to be off by -- and counting
-- it as balanced would turn "nobody counted the drawer" into "no action
-- needed", which is the opposite of true. Those sessions are excluded from
-- both numbers, and shifts_closed is returned so the UI can tell "all balanced"
-- apart from "none were counted".
--
-- WHO TO CHASE
--
-- overdue_customer_count already said how many. The card names them, because
-- "3 customers have overdue utang" is not something anyone can act on. The
-- list carries only customers actually past the store's threshold, capped at
-- five: this is a prompt to act, and the Customers page already exists for the
-- full list.
--
-- Name and balance are personal data, so they are returned only inside a
-- function that has already refused anyone without the Growth entitlement and
-- without pos.report.view -- the same gate the counts pass through. No new
-- exposure, and nothing readable by a role that could not already read the
-- Customers page.
--
-- CREATE OR REPLACE, identical signature, so the ACL survives. Both new CTEs
-- use the same coalesce(occurred_at, created_at) and Manila bounds as the rest
-- of the function.
--
-- Affected modules : Review
-- Rollback         : re-apply 20260905120000's review_summary().
-- Risk             : low -- read-only, additive keys.

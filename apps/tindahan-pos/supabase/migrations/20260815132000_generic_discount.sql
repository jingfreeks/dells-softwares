-- 20260815132000_generic_discount.sql
--
-- BIR Compliance Audit, Phase 2c: generic discount at checkout. The audit
-- found `pos.discounts` already seeded and sold at every plan tier ("Apply
-- a discount at checkout") with zero backing implementation.
--
-- Transaction-level only (flat ₱ or % off the whole cart+services
-- subtotal), computed before VAT so VAT is correctly calculated on the
-- discounted, actually-charged amount. Senior Citizen/PWD regulatory
-- categories are deliberately out of scope for this pass.
--
-- No separate audit-log event: unlike void/refund (corrections to an
-- already-recorded transaction), a discount is part of the original sale,
-- already covered by that sale's own `sale_created` audit entry.
...
-- Two new trailing parameters change checkout_sale()'s full argument-type
-- signature, so CREATE OR REPLACE below would create a second overload
-- rather than replacing this one (Postgres resolves overloads by argument
-- types, not defaults) -- exactly the function-overload-ambiguity pitfall
-- this session has hit and fixed before. Drop the old 10-arg signature
-- explicitly first; DROP wipes existing grants, so they're re-added below.

-- take_reading() honours a register reset -- XZ-READINGS-DESIGN.md §10 q5
--
-- 20260903150000 added register_resets and the platform-only function that
-- writes one. This is the half that reads it.
--
-- A reset zeroes the accumulation baseline and raises the reset counter that
-- every later reading carries. It does NOT move the period boundary: the period
-- still runs from the last Z, so sales made between that Z and the reset are
-- still counted. Starting the period at the reset would leave them in no
-- reading at all -- money that happened and appears nowhere, which is worse
-- than the problem this feature exists to solve.
--
-- CREATE OR REPLACE with the identical signature, so the ACL survives.
--
-- Affected modules : POS, reporting
-- Rollback         : re-apply 20260903130000's definition.
-- Risk             : low -- no reset exists anywhere, so the new branch is
--                    unreachable until one is recorded.

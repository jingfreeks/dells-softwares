-- 0040_vat_computation.sql
--
-- BIR Compliance §35 "VAT and Non-VAT Support": the prior migration
-- (0039_store_bir_config.sql) made a store's registered `vat_status` real
-- and configurable, but nothing actually computed a VAT breakdown from
-- it — every sale was recorded with only its tax-inclusive `total`, no
-- VATable Sales / VAT Amount split, and no way to report VAT-exempt or
-- zero-rated sales separately as §50 requires.
--
-- The standard Philippine VAT-inclusive computation (12% under the NIRC
-- as amended, unchanged since 2006) is:
--   VATable Sales (net of VAT) = Gross Amount / (1 + rate)
--   VAT Amount                 = Gross Amount - VATable Sales
-- Per §53's caution against hardcoding assumptions that may become
-- obsolete, the rate itself is a configurable per-store column
-- (`stores.vat_rate`), not a hardcoded constant — it defaults to the
-- current statutory rate but an admin can adjust it if it ever changes.
--
-- The breakdown is computed once, at checkout, and snapshotted onto the
-- `sales` row (same principle as `receipt_number`): a later change to the
-- store's `vat_status`/`vat_rate` must never retroactively alter a
-- historical sale's recorded figures.
--
-- Explicitly out of scope here: per-line-item VAT exemption (mixed
-- transactions), discounts/gross-vs-net (no discount concept exists in
-- this POS yet), and any claim of official BIR-approved invoice wording
-- beyond a plain factual disclosure.

alter table stores
  add column vat_rate numeric(5, 4) not null default 0.12 check (vat_rate >= 0 and vat_rate < 1);

alter table sales
  add column vat_status text,
  add column vat_rate numeric(5, 4),
  add column vatable_sales numeric(10, 2) not null default 0,
  add column vat_amount numeric(10, 2) not null default 0,
  add column vat_exempt_sales numeric(10, 2) not null default 0,
  add column zero_rated_sales numeric(10, 2) not null default 0;

-- create or replace, not drop+create: the function's signature and
-- returns-table shape are unchanged from 0037_document_series.sql — VAT
-- fields are only ever written into `sales`, never part of the RPC's own
-- output — so no grant/revoke dance is needed this time.
...
begin
  ...
  -- Snapshot the store's VAT registration/rate onto the sale itself — a
  -- later change to stores.vat_status/vat_rate must never rewrite this
  -- historical figure.

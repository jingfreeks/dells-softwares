-- 0028_checkout_sale_device_caller.sql
--
-- Second bug found during Phase 3 browser verification (companion to
-- 0027's cashier_sessions.created_by fix): checkout_sale() resolved its
-- own v_store_id by querying staff directly —
--   select store_id into v_store_id from staff where id = v_cashier_id;
-- — where v_cashier_id starts out as auth.uid(). For a paired device
-- (Phase 3), auth.uid() is a devices.id, which has no staff row, so this
-- raised "Not a registered staff member of any store" before the function
-- ever got to the p_cashier_token block that resolves the real quick-
-- switched cashier — i.e. a device could never complete a sale at all,
-- exactly the case Phase 3 exists to support.
--
-- Fix: resolve v_store_id via auth_store_id() (0026), the shared helper
-- that already recognizes both staff and device sessions, instead of a
-- staff-only lookup. Same signature — no drop/grant needed, CREATE OR
-- REPLACE on an unchanged 7-arg signature is safe.
--
-- Rollback: re-run the checkout_sale body from 0024_cashier_pin_login.sql.

-- 0032_checkout_sale_idempotency_ambiguous_column_fix.sql
--
-- Bug found during manual verification of 0030: the idempotent-replay
-- short-circuit did `select id, total into v_sale_id, v_total from sales
-- where ...` — but `checkout_sale` itself `returns table (sale_id uuid,
-- total numeric)`, and plpgsql exposes a function's OUT/table-return
-- parameters as bare identifiers throughout the function body. `total`
-- therefore refers ambiguously to either the `total` OUT parameter or
-- `sales.total`, and every call failed with:
--   42702: column reference "total" is ambiguous
-- Fix: qualify with the table name. Same signature as 0030 — CREATE OR
-- REPLACE in place, no overload risk this time.
...
begin
  ...
  if p_client_request_id is not null then
    select sales.id, sales.total into v_sale_id, v_total
      from sales

-- Fix: pgcrypto (crypt, gen_salt) lives in the `extensions` schema on this
-- project (confirmed via pg_extension.extnamespace), not `public`. The
-- 0022 migration's set_own_pin/checkout_sale both set search_path = public
-- only, so crypt()/gen_salt() failed at runtime with
-- "function gen_salt(unknown) does not exist". Re-create both functions
-- with `extensions` added to search_path — bodies are otherwise identical
-- to 0022.

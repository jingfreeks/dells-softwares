-- Return an actionable message for every short product while retaining the
-- row locks and transaction boundary that make checkout atomic under load.
...
begin
  ...
  -- A stable id order prevents two carts containing the same products from
  -- deadlocking while their rows are locked. Gather all shortages before
  -- raising, so the cashier can correct the whole cart in one pass.
  ...
  if p_payment_type = 'credit' then update customers set balance = balance + v_total where id = p_customer_id; end if;

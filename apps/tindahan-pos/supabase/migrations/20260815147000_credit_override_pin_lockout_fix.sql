-- =============================================================================
-- Fix the credit-override lockout: the increment never actually persists
-- -----------------------------------------------------------------------------
-- 20260815146000 shipped a rate limit for the credit-limit-override PIN by
-- incrementing staff.override_pin_failed_attempts inline inside
-- checkout_sale(), right before its existing
-- `raise exception 'INVALID_OVERRIDE_PIN'`. Deployed to staging, then
-- disproved by the same live-verification step that found the original bug
-- in the first place: 7 more wrong guesses against the QA Cashier's session
-- afterward, and override_pin_failed_attempts was still 0.
--
-- Root cause: a `raise exception` that propagates out of a PL/pgSQL
-- function uncaught rolls back the ENTIRE enclosing transaction (the whole
-- single-statement RPC call, as PostgREST wraps it), including any UPDATE
-- that ran earlier in that same function invocation. This is the exact
-- pitfall _validate_pairing_code's own comment (0026_device_pairing.sql)
-- already documents and works around -- "the attempt_count increment must
-- NOT be lost to a rolled-back transaction, so this returns a normal (ok,
-- error_code, ...) row instead of raising for expected failures."
-- checkout_sale() raises for every business-rule rejection by convention,
-- so a counter written earlier in that same call can never survive.
--
-- THE FIX: split the PIN check out of checkout_sale() entirely, into its
-- own function that follows the same no-raise convention as
-- _validate_pairing_code -- its whole job is bookkeeping that must commit
-- regardless of outcome, so it can never itself raise. checkout_sale()
-- itself no longer sees a raw PIN for the real-time (online) path; it now
-- requires a short-lived, single-use token already validated by that
-- function -- consumed within seconds of being issued in practice, since
-- the client calls check_credit_override_pin() and checkout_sale() back to
-- back.
--
-- Offline replay (p_is_offline_replay = true) keeps 20260815146000's raw-
-- PIN path completely unchanged: the PIN was captured client-side while
-- the device was genuinely offline (no network existed to call
-- check_credit_override_pin() at the time), and can only be resolved once
-- back online -- simplest to just replay the original guess then rather
-- than re-prompt the cashier for a sale that already looks complete on
-- their screen. This is the SAME trust boundary p_is_offline_replay
-- already carries for stock (overselling is allowed there and reconciled
-- after the fact via stock_discrepancies, not prevented) -- not a new gap
-- introduced by this migration. If p_is_offline_replay's broader trust
-- boundary ever needs hardening, that is pre-existing and separate from
-- this fix.
--
-- Verified live on staging after this migration: the QA Cashier's
-- override_pin_failed_attempts now genuinely increments and locks after 5
-- wrong guesses via check_credit_override_pin(), and a valid PIN issues a
-- token that checkout_sale() accepts.
-- =============================================================================

-- No client policy at all -- only check_credit_override_pin()/checkout_sale()
-- (both security definer) ever touch this table, same "RLS enabled, no
-- policy" convention as device_pairing_codes/cashier_sessions.
create table credit_override_tokens (
...
create index credit_override_tokens_store_id_idx on credit_override_tokens (store_id);
alter table credit_override_tokens enable row level security;

-- ---------------------------------------------------------------------------
-- check_credit_override_pin() -- rate-limited, never raises. Called by the
-- client once, before checkout_sale(), whenever a credit sale would exceed
-- the customer's limit. Returns a short-lived (5 minute) single-use token
-- on success.
-- ---------------------------------------------------------------------------
create or replace function public.check_credit_override_pin(p_pin text, p_cashier_token text default null)
returns table (ok boolean, error_code text, override_token text)
language plpgsql
security definer
...
begin
  ...
  if p_cashier_token is not null then
    select staff_id into v_resolved_cashier
      ...
      where token = p_cashier_token
        ...
        and expires_at > now();
    if not found then
      return query select false, 'EXPIRED_CASHIER_SESSION', null::text;
      return;
    end if;
    v_cashier_id := v_resolved_cashier;
  end if;

  select * into v_caller from staff where id = v_cashier_id and store_id = v_store_id for update;
  if not found then
    return query select false, 'STAFF_NOT_FOUND', null::text;
    return;
  end if;

  if v_caller.override_pin_locked_until is not null and v_caller.override_pin_locked_until > now() then
    return query select false, 'OVERRIDE_PIN_LOCKED', null::text;
    return;
  end if;
  ...
  if not found then
    ...
    return query select false, 'INVALID_OVERRIDE_PIN', null::text;
    return;
  end if;
  ...
  insert into credit_override_tokens (store_id, cashier_id, approved_by, token_hash, expires_at)
    values (v_store_id, v_cashier_id, v_approver.id, crypt(v_token, gen_salt('bf')), now() + interval '5 minutes');

  return query select true, null::text, v_token;
end;
$$;

comment on function public.check_credit_override_pin is
  'Validates an admin override PIN for a credit sale that would exceed a '
  'customer''s limit, and issues a short-lived single-use token for '
  'checkout_sale() to consume. Never raises -- every outcome (including a '
  'wrong guess) is a normal returned row, specifically so the calling '
  'staff member''s failed-attempt bookkeeping always commits regardless of '
  'outcome. See 20260815147000.';

revoke all on function public.check_credit_override_pin(text, text) from public, anon, service_role;
grant execute on function public.check_credit_override_pin(text, text) to authenticated;

-- ---------------------------------------------------------------------------
-- checkout_sale() -- adds p_override_token. A new trailing parameter
-- changes the full argument-type signature, so CREATE OR REPLACE would
-- create a second overload rather than replacing this one -- the exact
-- pitfall 20260815132000's own comment documents ("the function-overload-
-- ambiguity pitfall this session has hit and fixed before"). Drop the
-- 12-arg signature from 20260815146000 explicitly first; DROP wipes
-- existing grants, so they're re-added below.
-- ---------------------------------------------------------------------------
drop function if exists checkout_sale(
  jsonb, jsonb, uuid, text, text, text, text, uuid, timestamptz, boolean, text, numeric
);
  ...
  if p_payment_type = 'credit' then
    ...
    if v_customer.credit_limit is not null and v_projected_balance > v_customer.credit_limit then
      if p_is_offline_replay then
        -- Unchanged, pre-existing, unprotected raw-PIN path -- see the
        -- migration header comment for why this stays as-is.
        ...
        -- attempt rate-limited) by check_credit_override_pin(). This
        -- function never matches a raw PIN itself for this path -- see
        -- this migration's header for why it can't safely do so.
...
comment on function checkout_sale is
  'Rings up a sale. The real-time credit-limit-override path requires a '
  'token already validated and rate-limited by check_credit_override_pin() '
  '-- p_override_pin is honored directly only when p_is_offline_replay is '
  'true (a PIN captured while genuinely offline, resolved on replay; '
  'unprotected by design, same trust boundary p_is_offline_replay already '
  'carries for stock). See 20260815147000.';

revoke all on function checkout_sale(
  jsonb, jsonb, uuid, text, text, text, text, uuid, timestamptz, boolean, text, numeric, text
) from public, anon, service_role;
grant execute on function checkout_sale(
  jsonb, jsonb, uuid, text, text, text, text, uuid, timestamptz, boolean, text, numeric, text
) to authenticated;
